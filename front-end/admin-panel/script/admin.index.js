...
async function show_data(category) {
       let request = await fetch(`http://localhost:8080/products`);
       ...
       let show_data = data.map((element) => {
              ...
              <div class="products">
                     <div> 
                            <img src="${element.imgage}" alt="" id="img-src">
                     </div>
                     <div>
                            ...
                            <button class="yl_btn remove" onclick="handleDelete(${element._id})">Remove</button>
                     </div>
              </div>
              `
       });
...
async function handleDelete(id) {
       ...
       let request = await fetch(`http://localhost:8080/products/${id}`, {
              method: "DELETE",
              headers: {
                     "Content-Type": "application/json"
              }
